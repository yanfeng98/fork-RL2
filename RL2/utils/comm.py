...
def split_and_scatter_list(lst: list[dict[str, torch.Tensor]], device_mesh: dist.device_mesh.DeviceMesh) -> list[dict[str, torch.Tensor]]:
    ...
    lst: list[list[dict[str, torch.Tensor]]] = [None]
...
def boardcast_list(lst: list[dict[str, torch.Tensor]], device_mesh: dist.device_mesh.DeviceMesh) -> list[dict[str, torch.Tensor]]:

    kwargs: dict[str, Any] = {
        "group": device_mesh.get_group(),
        "group_src": 0
    }
    length: torch.LongTensor = torch.LongTensor([
        len(lst)
        if device_mesh.get_local_rank() == 0
        else 0
